-- ============================================================
-- PostgreSQL conversion of the CRM script (originally MySQL)
--
-- NOTES ON KEY CHANGES:
-- 1. Postgres cannot run CREATE/DROP DATABASE inside a script that
--    also creates tables in that DB, and there's no "USE" statement.
--    Connect to the target DB first (see instructions below), then
--    run the rest of this script against that connection.
-- 2. AUTO_INCREMENT -> SERIAL
-- 3. DATETIME -> TIMESTAMP
-- 4. Column aliases 'like this' -> "like this" (Postgres uses double
--    quotes for identifiers/aliases; single quotes are string literals)
-- 5. IFNULL -> COALESCE
-- 6. IF(cond, a, b) -> CASE WHEN cond THEN a ELSE b END
-- 7. JSON_OBJECT/JSON_ARRAYAGG/JSON_ARRAY -> jsonb_build_object/jsonb_agg
-- 8. SUBSTRING_INDEX has no direct equivalent -> rewritten with
--    string_to_array / split_part
-- 9. ALTER TABLE ... ADD COLUMN (a, b) -> separate ADD COLUMN clauses
-- 10. Division must be cast to numeric to avoid integer truncation
-- ============================================================
...
VALUES ('Code Backend', '2026-05-02', '2026-05-20', 'Đã tét', '2026-05-19 01:20:30', 1, 1, 3),
       ...
       ('Integration test', '2026-05-16', '2026-05-20', 'So hard', '2026-05-19 01:00:00', 2, 1, 3),
...
UPDATE users SET last_name = 'Nguyen' WHERE id = 1;
UPDATE users SET last_name = 'Nguyen' WHERE id = 2;
UPDATE users SET last_name = 'Nguyen' WHERE id = 3;
UPDATE users SET last_name = 'Nguyen' WHERE id = 4;

UPDATE users SET first_name = 'Van Mot' WHERE id = 1;
